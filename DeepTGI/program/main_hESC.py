...
def prepare():
    ...
    extra_gene=pd.read_csv('~/zhongle/human/hHep/expr.csv')
    print(extra_gene)
    ...
    pair_label = pd.read_csv('~/zhongle/human/hHep/label.csv')
...
def cross_val(feature, label, event_num):
    ...
    for train_index, test_index in skf.split(feature, label):
        ...
        mat_GENIE3=pd.read_csv('~/test_R/pred_nets/GENIE3/GENIE-hESC1.csv',sep=' ')
        mat_PPCOR=pd.read_csv('~/test_R/pred_nets/PPCOR/PPCOR-hESC.csv',sep=' ')
        mat_PIDC=pd.read_csv('~/test_R/pred_nets/PIDC/PIDC-hESC.csv',sep=' ')
        
        model = BERT(len(feature[0]), bert_n_heads, bert_n_layers, event_num)
        
        label_all=np.array(pd.read_csv('~/zhongle/human/hESC/label.csv'))
